...
// ========== 机器人自己消息记录（插件层独立记录，不依赖框架） ==========
/**
 * 框架发送群消息成功后的回调钩子
 * 由 bot.php 的 云雀API() 在发送成功后自动调用
 * 独立记录机器人自己的消息 ID（供撤回）和消息内容（供查询/审计）
 *
 * @param string $群号      群 openid
 * @param string $消息ID    发送接口返回的消息 id
 * @param string $消息内容  消息文本/MD 摘要
 * @param int    $消息类型  0=文字 2=MD 7=富媒体
 * @param int    $时间戳    发送时间
 */
function 机器人消息钩子($群号, $消息ID, $消息内容, $消息类型, $时间戳) {
    $群号 = trim((string)$群号);
    $消息ID = trim((string)$消息ID);
    if ($群号 === '' || $消息ID === '') return;

    // 机器人自身 openid
    $botId = defined('机器人ID') ? (string)机器人ID : (string)机器人ID();
    if ($botId === '') return;

    // ---- 1. 记录消息 ID 到群消息缓存（供"撤回 @机器人"使用）----
    $缓存键 = "群管/消息缓存/" . $群号;
    ...
    $列表 = 读($缓存键, $botId, []);
    if (!is_array($列表)) $列表 = [];
    // 防止重复记录（框架已记过一次，这里做冗余但不重复）
    if (empty($列表) || $列表[0] !== $消息ID) {
        array_unshift($列表, $消息ID);
        $列表 = array_slice($列表, 0, 30);
        写($缓存键, $botId, $列表);
    }

    // ---- 2. 记录消息内容到机器人消息日志（供查询/审计）----
    $类型名 = [0 => '文字', 2 => 'MD', 7 => '富媒体'][$消息类型] ?? '其他';
    $日志键 = "群管/机器人消息/" . $群号;
    $日志 = 读($日志键, "list", []);
    if (!is_array($日志)) $日志 = [];
    // 避免重复记录同一条消息
    ...
    if (!$已存在) {
        array_unshift($日志, [
            'id'      => $消息ID,
            'content' => mb_substr((string)$消息内容, 0, 500),
            'type'    => $类型名,
            'time'    => date('Y-m-d H:i:s', (int)$时间戳),
        ]);
        $日志 = array_slice($日志, 0, 50);
        写($日志键, "list", $日志);
    }
}
...
if (消息来源 === "群聊") {
    ...
    if (消息来源 === "群聊" && !$是否管理命令) {
        ...
        $用户消息列表 = 读($缓存键, $发言用户, []);
        if (!is_array($用户消息列表)) $用户消息列表 = [];
        // 防止重复记录
        if (empty($用户消息列表) || $用户消息列表[0] !== $当前消息ID) {
    ...
    // ========== 查询机器人最近消息 ==========
    if ($纯命令 == "机器人消息") {
        $机器人消息列表 = 读("群管/机器人消息/".$群号, "list", []);
        if (empty($机器人消息列表)) {
            文字("暂无机器人消息记录（机器人发出的消息会自动记录，保留最近50条）。");
            return;
        }
        $msg = "🤖 机器人最近消息（共".count($机器人消息列表)."条）\n\n";
        foreach (array_slice($机器人消息列表, 0, 10) as $i => $项) {
            $内容 = $项['content'] ?? '';
            $内容 = str_replace(["\r", "\n"], ' ', $内容);
            $内容 = mb_substr($内容, 0, 60);
            $msg .= ($i+1)."[{$项['type']}][{$项['time']}]\n{$内容}\n\n";
        }
        $msg .= "💡 发送「撤回 @机器人」可撤回最新一条。";
        文字($msg);
        return;
    }
